...
LevelSelectable::LevelSelectable(string nname, Font *nfont, int nid, Texture* ntexture, RenderWindow* nwin) : window(nwin)
{
	...
	label.setPosition(Vector2f( 1.5f * window->getSize().x / 10.f, (float)(id+0.35f) * window->getSize().y / 5.f));
	box.setPosition(0.5f * window->getSize().x / 10.f, (float)(id+0.1f)* window->getSize().y / 5.f);
	...
	play.setPosition(Vector2f( 8.5f * window->getSize().x / 10.f, (float)(id+0.4f) * window->getSize().y / 5.f));
